...
def check_group(poses, grid) :
    ...
    for pose in poses :
        ...
        price = 0
        #up side
        if ((y == 0) or ((y-1, x) not in poses)) and \
            (((x == 0) or ((y, x-1) not in poses)) or \
             ((x != 0) and (y != 0 )and ((y-1,x-1) in poses))):
            price += 1
        #right side
        if ((x == (len(grid[y]) - 1)) or ((y, x+1) not in poses)) and \
            (((y == 0) or ((y-1, x) not in poses)) or \
             (x != (len(grid[y])-1) and (y != 0) and ((y-1, x+1) in poses))) :
            price += 1
        #down side
        if ((y == (len(grid) - 1)) or ((y+1, x) not in poses)) and \
            (((x == (len(grid[y]) - 1)) or ((y, x+1) not in poses)) or \
             ((x != len(grid[y])-1) and (y != len(grid)-1) and ((y+1, x+1) in poses))) :
            price += 1
        #left side
        if ((x == 0) or ((y, x-1) not in poses)) and \
            (((y == (len(grid) - 1)) or ((y+1, x) not in poses)) or \
             ((x != 0) and (y != len(grid)-1) and ((y+1, x-1) in poses))) :
            price += 1   
        sum += price
